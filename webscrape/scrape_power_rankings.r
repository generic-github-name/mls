# Scrape mlssoccer.com for historical power rankings
# output files saved in the current working directory

# THIS ISN'T WORKING (3/21/2017)

# set up R
rm(list=ls())
library(data.table)
library(stringr)
library(RSelenium) # for getting around the "load more" button
library(rvest)
library(httr) # for "use_proxy"

# use RSelenium to get around the load more
checkForServer()
startServer()
remDr <- remoteDriver()
remDr$open()
url = 'http://www.mlssoccer.com/series/power-rankings'
remDr$navigate(url)
...
links_scr = htmlpage %>%  
	html_nodes('.node-title a')
links = as.character(links_scr)
...
mlsScraper = function(year, club) {
	...
	# scrape
	url = paste0('http://www.mlssoccer.com/post/2017/03/21/mls-power-rankings-week-3-orlando-jump-after-perfect-start')
	proxy.string = use_proxy("")
	htmlpage = html_session(url, proxy.string)
