...
#   mutate(Shtp96 = round(shots/totalMinutes*96,2),
#          xGp96    = round(xG/totalMinutes*96,2),
#          KPp96     = round(keyPasses/totalMinutes*96,2),
#          xAp96    = round(xA/totalMinutes*96,2),
#          `xG+xAp96  = round(xGxA/totalMinutes*96,2)) %>%
#   rename() # Started to fix this, then realized it was being written into HTMLOutputs in the pipeline
# 
# 
...
#                                      print.xtable(xtable(withMinutes, 
...
teamStats <- teamStats %>% 
                    ...
                    gd = gf - ga, 
                    `GD-xGD` = gd - xGD) %>%
  select(c('team','gp','xGF','xGA','xGD','gf','ga','gd','GD-xGD')) %>%
  arrange(desc(xGD))
...
write.table(gsub("\n", "",
                 gsub(" <", "<", 
                      gsub("> ", ">", 
                           gsub(' align="right"', "",
                                gsub("table border=1", 'table border=1 class = "sortable"',
                                     print.xtable(xtable(statsWithTeams, 
