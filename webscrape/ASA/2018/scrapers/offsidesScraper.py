...
#Scrape whoscored json for raw clearances
...
def main():
	...
	f = open('raw offsides.csv','w+')

	header= "date,gameID,eventID,time,half,player,team,action,team.1,outcome,x,y,hteam,ateam,hscore,ascore,hplayers,aplayers,hfinal,afinal,final,endX,endY,playerOffside,isLongball,passEndX,passEndY)"
	print(header,file=f)
			...
			try: 
				...
				if eID in [2,55]: 
					if eID == 2:
						...
						isLongball = "0"
						for q in e["qualifiers"]:
							if q["type"]["value"] == 7: #PlayerCaughtOffside
								playerOffside = playerDict[str(q["value"])]
							elif q["type"]["value"] == 140: #PassEndX
								passEndX = str(q["value"])
							elif q["type"]["value"] == 141: #PassEndY
								passEndY = str(q["value"])
							elif q["type"]["value"] == 1: #Longball
								isLongball = "1"
					elif eID == 55:
