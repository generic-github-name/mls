...
shots <- data.frame(read.csv('raw shots.csv',stringsAsFactors=FALSE))
shots$passID <- NULL
shots.with.xg <-   data.frame(read.csv('shots with xG.csv',stringsAsFactors = FALSE))
shots.with.xg$X <- NULL

shots <- arrange(shots,date,gameID,half,time) %>%
    filter(!(eventID %in% shots.with.xg$eventID))
...
gmloc <- function(data, flength = 115, fwidth = 80){
    X <- data[1]
    Y <- data[2]
    gmX <- data[3]
    X <- (100 - X)*flength/100
