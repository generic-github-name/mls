...
def main():
	...
	header= "date,gameID,eventID,time,half,ref,player,team,team.1,yellowCard,secondYellow,redCard,hteam,ateam,hscore,ascore,hplayers,aplayers,hfinal,afinal,final"
	print(header,file=f)
	...
	for gameIDL in idList:
		...
		for e in events:
			...
			try: 
				...
				if eID == 17 and "playerId" in e.keys():
					...
					red = 0
					secondYellow = 0
					if e['cardType']['value'] == 31:
						yellow = 1
					elif e['cardType']['value'] == 32:
						secondYellow = 1
					elif e['cardType']['value'] == 33:
						red = 1
					for q in e['qualifiers']:
						#second yellow or red, respectively, and check to make sure its not the coach
						if q['type']['value'] == 32 or q['type']['value'] == 33:
							if teamid == homeID:
								hplayers = hplayers - 1
							else:
								aplayers = aplayers - 1
					line = str(date)+','+str(gameID) +','+ str(eventID) +','+ str(minute)+':'+str(second)+','+str(half)+ ',' + unidecode(ref)+','+unidecode(player)+','+str(team)+ ',' +str(team1) + ',' + str(yellow) + ',' + str(secondYellow) + ',' + str(red) +','+str(homeTeam)+','+str(awayTeam)+','+str(hscore)+','+str(ascore)+','+str(hplayers)+','+str(aplayers)+','+str(hfinal)+','+str(afinal)+','+str(final)
					print(line,file=f)
