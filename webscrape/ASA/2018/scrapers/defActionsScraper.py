...
def main():
	...
	f = open('raw defensive actions.csv','w+')
		#s = open('tracker.csv','w+')
	header= "date,gameID,eventID,time,half,player,team,action,team.1,outcome,x,y,hteam,ateam,hscore,ascore,hplayers,aplayers,hfinal,afinal,final"
	print(header,file=f)
	...
	for gameIDL in idList:
		...
		for e in events:
			...
			try: 
				...
				if eID in [7,8,45] or block == 1 or blockedCross == 1: 
					if eID == 7:
						action = "tackle"
					elif eID == 8:
						action = "interception"
					...
					line = str(date)+','+str(gameID) +','+ str(eventID) +','+ str(minute)+':'+str(second)+','+str(half)+','+unidecode(player)+','+str(team)+ ',' + action + ',' +str(team1)+','+str(success)+ ',' +str(xCoord)+','+str(yCoord) +','+str(homeTeam)+','+str(awayTeam)+','+str(hscore)+','+str(ascore)+','+str(hplayers)+','+str(aplayers)+','+str(hfinal)+','+str(afinal)+','+str(final)
					print(line,file=f)
