...
def main():
	...
	f = open('raw clearances.csv','w+')
		#s = open('tracker.csv','w+')
	header= "date,gameID,eventID,time,half,player,team,action,team.1,outcome,x,y,hteam,ateam,hscore,ascore,hplayers,aplayers,hfinal,afinal,final,endX,endY,headed"
	print(header,file=f)
	...
	for gameIDL in idList:
		...
		for e in events:
			...
			try: 
				...
				if eID in [12,74] and blockedCross == 0: 
					if eID == 12:
						action = "clearance"
					elif eID == 74:
						action = "blockedPass"
					...
					line = str(date)+','+str(gameID) +','+ str(eventID) +','+ str(minute)+':'+str(second)+','+str(half)+','+unidecode(player)+','+str(team)+ ',' + action + ',' +str(team1)+','+str(success)+ ',' +str(xCoord)+','+str(yCoord)+','+str(homeTeam)+','+str(awayTeam)+','+str(hscore)+','+str(ascore)+','+str(hplayers)+','+str(aplayers)+','+str(hfinal)+','+str(afinal)+','+str(final)+','+str(endX)+','+str(endY)+','+str(headed)
					print(line,file=f)
