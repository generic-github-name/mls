...
#NOTE - I AM LEAVING OUT AERIALS THAT RESULT IN FOULS
...
def main():
	...
	f = open('dribbles.csv','w+')
		#s = open('tracker.csv','w+')
	header= "date,gameID,eventID,time,half,player,team,team.1,success,x,y,hteam,ateam,hscore,ascore,hplayers,aplayers,hfinal,afinal,final"
	print(header,file=f)
		...
		for e in events:
			...
			try: 
				...
				if eID == 3:
					...
					line = str(date)+','+str(gameID) +','+ str(eventID) +','+ str(minute)+':'+str(second)+','+str(half)+','+unidecode(player)+','+str(team) + ',' +str(team1)+ ',' +str(success) +','+str(xCoord)+','+str(yCoord) +','+str(homeTeam)+','+str(awayTeam)+','+str(hscore)+','+str(ascore)+','+str(hplayers)+','+str(aplayers)+','+str(hfinal)+','+str(afinal)+','+str(final)
					print(line,file=f)
