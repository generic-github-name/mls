...
# ADD LABELS ABOUT MNINUTES ON/OFF
...
# keep after 2016
boxScores = boxScores[year(date)>=2016]
...
boxScores = boxScores[total_minutes>m]
...
players = unique(boxScores$player)
players = players[players %in% boxScores[year(date)==2017]$player]
...
for(p in players) {
	...
	gridData[[p]] = data.table(expand.grid(seq(0,100,by=r), seq(0,100,by=r), unique(data[[var]])))
...
for(p in players) {
	var = gsub(' ', '_', p)
	cfData[[p]] = data.table(expand.grid(seq(0,100, by=r2), seq(0,100, by=r2), c(TRUE, FALSE)))
...
for(p in players) {
	...
	p1 = ggplot(cfData[[p]][wrap==paste('With', p)], aes(y=y,x=x,fill=pred_nb*f)) + 
		...
		labs(title=paste('With', p), y='Defensive End') + 
		theme_minimal()	+ 
	...
	p2 = ggplot(cfData[[p]][wrap==paste('Without', p)], aes(y=y,x=x,fill=pred_nb*f)) + 
		...
		draw_grob(lines, x=x, y=y, width=width, height=height) + # this is bad. I have to match the location/scale to the aspect ratio of the output
		scale_fill_gradientn(paste('Actions\n Per',f,'\nGames'), colors=colors) +
		labs(title=paste('Without', p), y='Defensive End') + 
		theme_minimal()	+ 
		theme(axis.title.x=element_blank(), axis.title.y=element_text(size=14), axis.text=element_blank(), 
		axis.ticks=element_blank(), plot.title=element_text(hjust=.5, size=16), line = element_blank(), 
		strip.text.x = element_text(size=14))
	
	plots[[p]] = grid.arrange(p1, p2, ncol=2)
}
